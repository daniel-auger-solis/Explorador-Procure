...
def latest_revisions(file_list):
    ...
    - Excludes files whose stem contains _CK (case-insensitive)
    - Excludes files with no revision suffix (no _A, _B, _0, _1, etc.)
    - Among files with a revision suffix, keeps only the latest per (folder, base, ext)
    """
    ...
    for rel, absp in file_list:
        ...
        # exclude _CK anywhere in the stem
        if re.search(r'_CK', stem, re.IGNORECASE):
            continue
        folder = os.path.dirname(absp)
        base, rev = _base_and_rev(stem)
        if rev is None:
            continue   # no revision suffix -> skip
        groups[(folder, base, ext.lower())].append((rev, rel, absp))
    result = []
    for key, entries in groups.items():
        entries.sort(key=lambda x: _rev_key(x[0]))
        best = entries[-1]
...
class FileBrowser(tk.Frame):
    ...
    def __init__(self, master, tile, close_cb):
        ...
        if not hasattr(tile, "saved_filters"):
            tile.saved_filters = []   # list of str
        # chip_filter persists on tile; auto-activate first filter on open
        if not hasattr(tile, "_active_chip"):
            tile._active_chip = tile.saved_filters[0] if tile.saved_filters else ""
        self._build()
        # _chip_filter property reads/writes tile._active_chip
        self._qvar.set("")
    ...
    def _build(self):
        ...
        for label, val in [("Todos","all"),("PDF","pdf"),("Office","office")]:
            tk.Radiobutton(type_frame, text=label, variable=self._type_filter,
                           value=val, command=self._apply_filters,
...
class FolderBoard(tk.Tk):
    ...
    def toggle_edit(self):
        ...
        if self.edit_mode:
            ...
            for b in (self.btn_tile,self.btn_panel,self.btn_label): b.pack(side="left",padx=3)
            ...
            for b in (self.btn_tile,self.btn_panel,self.btn_label): b.pack_forget()
    ...
    def _ctx_menu(self,e):
        ...
        tile=self._hit_tile(e.x,e.y); lbl=self._hit_label(e.x,e.y); pan=self._hit_panel(e.x,e.y)
        m=tk.Menu(self,tearoff=0,bg=SURFACE,fg=TEXT,
        ...
        for d in items:
            ...
            if kind=="tile":
                t=Tile(self,d["tid"],d["name"],d["path"],d["color"],
                       ...
                       d.get("saved_filters",[]))
                if d.get("saved_filters"):
                    t._active_chip = d["saved_filters"][0]
    ...
    def load(self):
        ...
        for bd in boards_data:
            ...
            all_items = (bd.get("tiles",[]) + bd.get("panels",[]) + bd.get("labels",[]))
